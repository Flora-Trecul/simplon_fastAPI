...
def get_learning_sessions(db: Session = Depends(get_db)):
	...
	if not db_ls.items:
		raise HTTPException(status_code = status.HTTP_404_NOT_FOUND, detail = "Aucune session de formation trouvée.")
	
	return db_ls


@router.get("/{learning_session_id}", response_model=LSResponse)
def get_learning_session(learning_session_id: int, db: Session = Depends(get_db)):
	...
	if not db_ls:
		raise HTTPException(status_code = status.HTTP_404_NOT_FOUND, detail = "Cette session de formation est introuvable.")
	
	return db_ls


@router.patch("/{learning_session_id}", response_model=LSResponse, status_code = status.HTTP_201_CREATED)
def udpate_learning_sessions(learning_session_id: int, schema: LSUpdate, db: Session = Depends(get_db)):
	...
	if schema.course_id:
		...
		if not db_course:
			raise HTTPException(status_code = status.HTTP_404_NOT_FOUND, detail = "Cette formation est introuvable.")
...
@router.delete("/{learning_session_id}", status_code = status.HTTP_204_NO_CONTENT)
...
@router.get("/{learning_session_id}/users", response_model = list[UserResponse])
def get_learning_session_users(learning_session_id: int, db: Session = Depends(get_db)):
	...
	if db_ls_users is None:
		raise HTTPException(status_code = status.HTTP_404_NOT_FOUND, detail = "Cette session de formation est introuvable.")
	elif db_ls_users == []:
		raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail = "Aucun utilisateur inscrit à cette session de formation.")
